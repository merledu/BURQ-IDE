...
#endif
#include <iostream>
class LoadMemory_api_t: public sim_api_t<VerilatorDataWrapper*> {
    public:
    LoadMemory_api_t(VLoadMemory* _dut) {
        dut = _dut;
        main_time = 0L;
        is_exit = false;
#if VM_TRACE
    ...
    void init_sim_data() {
        ...
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_MemoryRead)));
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_MemoryWrite)));
        sim_data.inputs.push_back(new VerilatorIData(&(dut->io_wrData)));
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_wrAddr)));
        sim_data.outputs.push_back(new VerilatorIData(&(dut->io_Data)));
        sim_data.signals.push_back(new VerilatorCData(&(dut->reset)));
        sim_data.signal_map["LoadMemory.reset"] = 0;
    }
#if VM_TRACE
...
static LoadMemory_api_t * _Top_api;
...
int main(int argc, char **argv, char **env) {
    ...
    VLoadMemory* top = new VLoadMemory;
    std::string vcdfile = "test_run_dir/examples/LoadMemory/LoadMemory.vcd";
